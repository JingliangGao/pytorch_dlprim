    ...
    double const min_lr)
{
    ...
    double const decay_ratio = static_cast<double>(iter - warmup_iters) /
                               static_cast<double>(lr_decay_iters - warmup_iters);
...
int main(int argc, char** argv)
{
    ...
    if (mode == "train")
    {
        ...
        torch::optim::AdamW optimizer(
            model->parameters(), torch::optim::AdamWOptions(learning_rate).betas({0.9, 0.99}));

        // <path-to-shakespeare_char>
        DataLoader loader(
            "/home/kylin/gjl/project/pytorch_dlprim/examples/data/shakespeare_char",
            batch_size,
            block_size);
    ...
    else if (mode == "sample")
    {
        ...
        catch (const c10::Error& e)
        {
            ...
            std::cerr << "  2. The model architecture in the checkpoint does not "
