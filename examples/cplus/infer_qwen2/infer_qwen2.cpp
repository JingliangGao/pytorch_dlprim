...
std::vector<int64_t> encode(const std::string &text) {
    ...
    for (unsigned char c : text) ids.push_back(c % VOCAB_SIZE);
...
std::string decode(const std::vector<int64_t> &ids) {
    std::string s;
    for (auto id : ids) s.push_back((char)(id % 256));
    return s;
}
...
struct RMSNormImpl : torch::nn::Module {
    ...
    RMSNormImpl(int64_t hidden_size_, double eps_ = 1e-8)
        : eps(eps_), hidden_size(hidden_size_) {
...
struct MultiHeadAttentionImpl : torch::nn::Module {
    ...
    torch::Tensor forward(const torch::Tensor &x, const c10::optional<torch::Tensor> &mask = c10::nullopt) {
        ...
        auto attn_out = torch::matmul(attn_probs, v).permute({0,2,1,3}).contiguous().view({B, T, C});
...
struct FeedForwardImpl : torch::nn::Module {
    ...
    FeedForwardImpl(int64_t hidden_size, int64_t mult = 4) {
        ...
        fc_in = register_module("fc_in",
            torch::nn::Linear(torch::nn::LinearOptions(hidden_size, inner).bias(true)));
        fc_gate = register_module("fc_gate",
                torch::nn::Linear(torch::nn::LinearOptions(hidden_size, inner).bias(true)));
        fc_out = register_module("fc_out",
                torch::nn::Linear(torch::nn::LinearOptions(inner, hidden_size).bias(true)));
...
struct QwenForCausalLMImpl : torch::nn::Module {
    ...
    QwenForCausalLMImpl(int64_t vocab_size, int64_t hidden_size,
    ...
    torch::Tensor forward(const torch::Tensor &input_ids,
        ...
        for (size_t i = 0; i < layers->size(); ++i) {
            ...
            if(!tb) {
                throw std::runtime_error("Failed to cast layers[" + std::to_string(i) + "] to TransformerBlockImpl");
            }
            x = tb->forward(x, mask);
        }
        x = ln_f->forward(x);
...
// -----------------------------
std::string generate_from_module(std::shared_ptr<QwenForCausalLMImpl> model,
