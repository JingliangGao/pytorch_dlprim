    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            test_loss += F.cross_entropy(
                output, target_dev, reduction="sum"
            ).item()  # sum up batch loss
            pred = output.to("cpu").argmax(
                dim=1, keepdim=True
...
def main():
    ...
    examples_dir = current_file.parent.parent
    dataset1 = datasets.MNIST(
        f"{examples_dir}/data", train=True, download=True, transform=transform
    )
    dataset2 = datasets.MNIST(f"{examples_dir}/data", train=False, transform=transform)
