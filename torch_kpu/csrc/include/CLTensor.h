...
struct CLMemAllocation
{
    ...
    CLMemAllocation(int id, cl::Context& ctx, std::int64_t length, std::int64_t os)
        : device_id(id), size(length), orig_size(os), buffer(ctx, CL_MEM_READ_WRITE, length)
    {
    }
...
class CLCache
{
  public:
    ...
    bool reuse_oversized_chunks =
        getenv("OPENCL_CACHE_OVERSIZED") && atoi(getenv("OPENCL_CACHE_OVERSIZED"));
    ...
    DevData& data(int i)
    {
        ...
        if (!res.ready)
        {
            ...
            res.queue = res.ctx.make_execution_context(
                res.enable_profiling ? CL_QUEUE_PROFILING_ENABLE : 0);
