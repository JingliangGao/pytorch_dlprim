...
{
    TORCH_CHECK(
        tt.device().type() == c10::DeviceType::PrivateUse1, "OpenCL device is required for tensor");
    // TORCH_CHECK(tt.numel() > 0,"Buffer is not valid for unallocated defvice");
    TORCH_CHECK(
        tt.getIntrusivePtr()->storage().nbytes() > 0,
...
torch::Tensor new_ocl_tensor(torch::IntArrayRef size, c10::Device dev, c10::ScalarType type)
{
    ...
    c10::Storage storage(
        c10::Storage::use_byte_size_t(), mem, CLContextManager::allocate(dev, mem));

    c10::DispatchKeySet ks =
        c10::DispatchKeySet{c10::DispatchKey::OpenCL, c10::DispatchKey::AutogradOpenCL};

    c10::intrusive_ptr<c10::TensorImpl> impl = c10::make_intrusive<c10::TensorImpl>(
        std::move(storage), ks, caffe2::TypeMeta::fromScalarType(type));
...
torch::Tensor new_tensor_as(dlprim::Shape const& s, torch::Tensor const& as)
{
    ...
    for (int i = 0; i < s.size(); i++)
        shape[i] = s[i];
    torch::Tensor result =
        new_ocl_tensor(c10::IntArrayRef(shape, s.size()), as.device(), as.dtype().toScalarType());
    return result;
}
...
dlprim::Tensor make_workspace(at::DataPtr& ws_ptr, size_t ws_size, c10::Device const& dev)
{
    ...
    if (ws_size)
    {
        ...
        ws = dlprim::Tensor(
            cl::Buffer((cl_mem)ws_ptr.get(), true), 0, dlprim::Shape(ws_size), dlprim::uint8_data);
    }
    return ws;
}
