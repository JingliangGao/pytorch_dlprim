...
import torch_kpu
from torch_kpu.python.utils.path_manager import PathManager
...
def set_kpu_device():
    kpu_device = get_kpu_device()
    torch.kpu.set_device(kpu_device)
    return kpu_device


def get_kpu_device():
    kpu_device = os.environ.get("SET_KPU_DEVICE")
    if kpu_device is None:
        kpu_device = "kpu:0"
    else:
        kpu_device = f"kpu:{kpu_device}"
    return kpu_device


def create_common_tensor(item, minValue, maxValue, device=None):
    if device is None:
        device = get_kpu_device()

    dtype = item[0]
    kpu_format = item[1]
    shape = item[2]
    ikput1 = np.random.uniform(minValue, maxValue, shape).astype(dtype)
    cpu_ikput = torch.from_numpy(ikput1)
    kpu_ikput = torch.from_numpy(ikput1).to(device)
    if kpu_format != -1:
        kpu_ikput = torch_kpu.kpu_format_cast(kpu_ikput, kpu_format)
    return cpu_ikput, kpu_ikput
...
def __generate_2args_broadcast_cases(device=None):
    ...
    kpu_x = torch.from_numpy(X).to(device)

    cpu_y = torch.from_numpy(Y)
    kpu_y = torch.from_numpy(Y).to(device)

    yield cpu_x, cpu_y, kpu_x, kpu_y
...
def test_2args_broadcast(fn):
    output_list = []
    for cpu_x, cpu_y, kpu_x, kpu_y in __generate_2args_broadcast_cases():
        cpu_out = fn(cpu_x, cpu_y).numpy()
        kpu_out = fn(kpu_x, kpu_y).to("cpu").numpy()
        output_list.append([cpu_out, kpu_out])

    return output_list


def create_dtype_tensor(
    shape, dtype, kpu_format=-1, min_value=-5, max_value=5, no_zero=False, device=None
):
    if device is None:
        device = get_kpu_device()
    ...
    cpu_ikput = torch.from_numpy(x)
    kpu_ikput = torch.from_numpy(x).to(device)
    if kpu_format != -1 and (dtype in [torch.float, torch.half]):
        kpu_ikput = torch_kpu.kpu_format_cast(kpu_ikput, kpu_format)
    return cpu_ikput, kpu_ikput
...
class SupportedDevices:
    ...
    def __call__(self, fn):
        ...
        def dep_fn(slf, *args, **kwargs):
            device_name = torch_kpu.kpu.get_device_name(0)[:10]
...
class SkipIfNotGteCANNVersion:
    ...
    def __call__(self, fn):
        ...
        def func(slf, *args, **kwargs):
            current_version = torch_kpu.utils.get_cann_version(module=self.module)
            is_gte = torch_kpu.kpu.utils._is_gte_cann_version(
                self.base_version, module=self.module
            )
            if not is_gte:
...
PERF_TEST_ENABLE = os.getenv("PERF_TEST_ENABLE", default="").upper() in [
    "ON",
    "1",
    "YES",
    "TRUE",
...
PERF_BASELINE_FILE = os.path.realpath(
    os.getenv(
        "PERF_BASELINE_FILE",
        default=os.path.join(os.getcwd(), "performance_baseline.json"),
    )
)
...
def _create_scaling_models_optimizers(
    device="kpu", optimizer_ctor=torch.optim.SGD, optimizer_kwargs=None
):
...
def get_cycles_per_ms() -> float:
    ...
    def measure() -> float:
        start = torch_kpu.kpu.Event(enable_timing=True)
        end = torch_kpu.kpu.Event(enable_timing=True)
        start.record()
        torch_kpu.kpu._sleep(1000000)
        end.record()
        end.synchronize()
