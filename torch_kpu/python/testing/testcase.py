...
from torch_kpu.testing.common_utils import set_kpu_device, is_iterable, iter_indices
from torch_kpu.testing.common_utils import PERF_TEST_ENABLE, PerfBaseline
...
class TestCase(expecttest.TestCase):
    ...
    def assertRtolEqual(
        self, x, y, prec=1.0e-4, prec16=1.0e-3, auto_trans_dtype=False, message=None
    ):
        def _assertRtolEqual(x, y, prec, prec16, message):
            def compare_res(pre, minimum):
                ...
                if not result_rtol.all() and not result_atol.all():
                    if (
                        np.sum(result_rtol == False) > size * pre
                        and np.sum(result_atol == False) > size * pre
                    ):
            ...
            if x.dtype not in dtype_list:
                self.fail(
                    "required dtype in [np.bool_, np.uint16, np.int16, "
        ...
        if PERF_TEST_ENABLE:
            ...
            if len(performanceResult.errors) == len(performanceResult.failures) == 0:
                ...
                if baseline and runtime > baseline * 1.2:
                    errMsg = (
                        "Performance test failed. Performance baseline: "
                        + str(baseline)
                        + "s, current time: "
                        + str(runtime)
                        + "s"
