...
static inline at::Tensor to_impl_dlprim(
    const at::Tensor& self, const c10::TensorOptions& options, bool non_blocking, bool copy)
{

    // memory_format default: Preserve on CPU-like backends?
    // 保持与上面样例一致的策略
    ...
    bool pin_out = non_blocking && (self.device().type() == c10::DeviceType::PrivateUse1) &&
                   options.device().is_cpu() && (options.layout() == c10::kStrided);

    // Preserve behavior: 如果用户要求 Preserve，并且 tensor 是 non-overlapping
    // AND dense， 则复制 strides；否则退化为建议的 memory_format
    ...
    ::std::optional<at::MemoryFormat> memory_format)
{
    ...
    if (dtype.has_value())
        opts_override = opts_override.dtype(*dtype);
    if (layout.has_value())
        opts_override = opts_override.layout(*layout);
    if (device.has_value())
        opts_override = opts_override.device(*device);
    if (pin_memory.has_value())
        opts_override = opts_override.pinned_memory(*pin_memory);
    ...
    // 4) dtype double on OCL fallback to float (consistent with to(dtype)
    // behavior)
    if (options.dtype() == at::ScalarType::Double &&
        options.device().type() == c10::DeviceType::PrivateUse1)
    {
        if (!CLContextManager::fp64(options.device().index()))
        {
            TORCH_WARN(
                "Device ocl:" + std::to_string(options.device().index()) +
                " does not support cl_khr_fp64, falling back to float");
    ...
    c10::MemoryFormat mf =
        memory_format.has_value()
            ? *memory_format
            : (self.device().type() == c10::DeviceType::PrivateUse1 ? c10::MemoryFormat::Contiguous
    ...
    // 8) If Preserve requested and tensor is non-overlapping and dense, use
    // empty_strided to preserve strides
    ...
    auto r =
        at::empty(self.sizes(), options.memory_format(mf).pinned_memory(pin_out), c10::nullopt);
    r.copy_(self, non_blocking);
    return r;
}

at::Tensor
to(const at::Tensor& self,
   c10::Device device,
   at::ScalarType dtype,
   bool non_blocking,
   bool copy,
   ...
   c10::optional<c10::MemoryFormat> optional_memory_format)
{
    ...
    if (dtype == at::ScalarType::Double)
    {
        ...
        {
            TORCH_WARN(
                "Device ocl:" + std::to_string(self.device().index()) +
                " does not support double, casting to float.");
