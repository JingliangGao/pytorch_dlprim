    ...
    at::IntArrayRef stride)
{
    ...
    // at::native::checkSetStorage(self, source.storage(), storage_offset, size,
    // stride);
    ...
    // ---- 5. Set sizes / strides ----
    // If stride is empty (e.g. { }), you may choose to make the tensor contiguous
    // automatically
    if (stride.size() > 0)
    {
    ...
    // If your OpenCL backend has a StorageDescHelper similar to NPU's, call it
    // here: StorageDescHelper::CopyDesc(self, source);

    // If no descriptor helper exists yet, this step can be skipped without
...
at::Tensor& set_(at::Tensor& self)
{
    ...
    // Create a StorageImpl: pass nullptr for allocator (your CLContextManager
    // does not provide get_allocator) Storage size is in bytes; use SymInt(0)
    // here
    c10::intrusive_ptr<c10::StorageImpl> ocl_storage_impl = c10::make_intrusive<c10::StorageImpl>(
