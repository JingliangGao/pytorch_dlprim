...
at::Tensor& leaky_relu_out(const at::Tensor& self, const Scalar& negative_slope, at::Tensor& out)
{
    ...
    dlprim::core::pointwise_operation(
        {x}, {y}, {slope}, "y0 = x0 > 0 ? x0 : w0 * x0;", getExecutionContext(self));
    ...
    at::Tensor& grad_input)
{
    ...
    double slope = negative_slope.to<double>();
    at::Tensor self_c = self.contiguous(), grad_input_c = grad_input.contiguous(),
    ...
    dlprim::core::pointwise_operation(
        {y, dy}, {dx}, {slope}, "y0 = x0 > 0 ? x1 : w0 * x1;", getExecutionContext(self));
