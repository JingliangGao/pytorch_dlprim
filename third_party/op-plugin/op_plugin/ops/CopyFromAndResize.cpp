...
at::Tensor _copy_from(const at::Tensor& self, const at::Tensor& dst, bool non_blocking)
{
        ...
        self.device().type() == c10::DeviceType::PrivateUse1)
    {
        ...
        self.device().type() == c10::DeviceType::CPU &&
        ...
        dst.device().type() == c10::DeviceType::PrivateUse1)
    {
        if (self.is_contiguous() && dst.is_contiguous())
        {
            dlprim::core::pointwise_operation_broadcast(
                {todp(self)}, {todp(dst)}, {}, "y0=x0;", getExecutionContext(self.device()));
        }
        else
        {
            ...
            dlprim::core::copy_strided(
                shape,
                buffer_from_tensor(self),
                src_offset,
                src_std,
                buffer_from_tensor(dst),
                tgt_offset,
                tgt_std,
                todp(self.dtype()),
                todp(dst.dtype()),
