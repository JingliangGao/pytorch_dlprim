...
at::Tensor& amax_out(const at::Tensor& self, IntArrayRef dim, bool keepdim, at::Tensor& out)
{
    ...
    std::string ext_val = dlprim::data_type_to_opencl_numeric_limit(X.dtype(), dlprim::dt_min_val);
    auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
        ...
        "y0=x0;",
        "reduce_y0 = " + ext_val + ";",
        std::string("reduce_y0 = ") + "max" + "(reduce_y0,y0);");
...
at::Tensor max(const at::Tensor& self)
{
    ...
    std::string y0 = dlprim::data_type_to_opencl_numeric_limit(X.dtype(), dlprim::dt_min_val);
    dlprim::ExecutionContext q = getExecutionContext(self);
    dlprim::Context ctx(q);
    auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
        ctx,
...
max_out(const at::Tensor& self, int64_t dim, bool keepdim, at::Tensor& max, at::Tensor& max_values)
{
    ...
    dlprim::Tensor X = todp(self_c);
    dlprim::Tensor Yval = todp(max_c);
    dlprim::Tensor Yind = todp(max_values_c);
    ...
    if (!max.is_contiguous())
        max.copy_(max_c);

    if (!max_values.is_contiguous())
        max_values.copy_(max_values_c);
