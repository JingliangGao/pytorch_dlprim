...
namespace
{
static at::Tensor get_bmm_mm_valid_tensor(
    at::Tensor const& t, bool& transposed, int& ld, bool& copied, int bmm, char /*M*/)
{
    auto sizes = t.sizes();
...
at::Tensor&
mm_bmm_out_nocheck(const at::Tensor& self, const at::Tensor& mat2, at::Tensor& out, int bmm)
{
    ...
    TORCH_CHECK(
        ...
        "A(" +
            std::to_string(M) + "," + std::to_string(Ka) + ")" + (At ? ".T" : "  ") + "*B(" +
            std::to_string(Kb) + "," + std::to_string(N) + ")=" + (Bt ? ".T" : "  ") + "C(" +
            std::to_string(Mc) + "," + std::to_string(Nc) + ")");

    TORCH_CHECK(
        A.dtype() == B.dtype() && A.dtype() == C.dtype(), "All matrices must have same dtype");
    if (bmm)
    {
        TORCH_CHECK(
            A.sizes()[0] == B.sizes()[0] && A.sizes()[0] == C.sizes()[0],
            "Matrices must have same batch i.e. 0 dimention");
    }
    ...
    if (bmm == 0)
    {
        ...
        gemm_op->gemm(
            M, N, K, Abuf, Aoff, lda, Bbuf, Boff, ldb, Cbuf, Coff, ldc, nullptr, 0, 0, M * N, q);
    }
    else
    {
        int batch = A.sizes()[0];
...
{
    return mm_bmm_out_nocheck(self, mat2, out, 1);
}
