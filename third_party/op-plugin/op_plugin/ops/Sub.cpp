...
at::Tensor&
sub_out(const at::Tensor& self, const at::Tensor& other, const Scalar& alpha, at::Tensor& out)
{
    ...
    if (is_cpu_scalar(other, value))
    {
        ...
        dlprim::core::pointwise_operation(
            {x0}, {y0}, {w0}, "y0 = x0 - w0;", getExecutionContext(self));
    }
    else if (is_cpu_scalar(self, value))
    {
        ...
        dlprim::core::pointwise_operation(
            {x0}, {y0}, {w0, w1}, "y0 = w0 - x0 * w1;", getExecutionContext(other));
    }
    else
    {
        at::Tensor self_c = self.contiguous();
        ...
        dlprim::core::pointwise_operation_broadcast(
            {x0, x1}, {y0}, {w0}, "y0 = x0 - x1 * w0;", getExecutionContext(self));
    }
