...
at::Tensor _adaptive_avg_pool2d(const at::Tensor& self, IntArrayRef output_size)
{
    ...
    TORCH_CHECK(
        (output_size[0] == 1 && output_size[1] == 1) ||
            (output_size[0] == h && output_size[1] == w),
        "Only global pooling or no-pooling supported");
    at::Tensor result;
    if (output_size[0] == 1 && output_size[1] == 1)
    {
        ...
        auto pool = dlprim::core::Pooling2DForward::create_global_avg_pooling(
            ctx, X.shape(), todp(self.dtype()));
        pool->enqueue(X, Y, q);
    }
    else
    {
