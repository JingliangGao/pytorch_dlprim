...
def gen_dispatchkey_return(f: NativeFunction, type: str) -> List[Optional[str]]:
    privateuse1_ret = set()
    ...
    with native_function_manager(f):
        ...
        if aten_name in AllPrivateUse1_Op:
            privateuse1_ret.add(
                f"""m.impl("{aten_name}", TORCH_FN(at_torch::{ns}::{impl_name}));"""
            )
            autogradprivateuse1_ret.add(
                f"""m.impl("aten::{op_name_with_overload}", TORCH_FN(at_torch::{ns}::{impl_name}));"""
            )
...
def parse_native_yaml(
    ...
    dispatch_registrations_body = sorted(
        set(concatMap(lambda f: gen_dispatch_return(f, "wrap_func"), res))
    )
    p_registrations_body = sorted(
        set(concatMap(lambda f: gen_dispatchkey_return(f, "privateuse1"), res))
    )
    ap_registrations_body = sorted(
        set(concatMap(lambda f: gen_dispatchkey_return(f, "autograd_privateuse1"), res))
    )

    return (
        backend_declarations,
        dispatch_registrations_body,
        p_registrations_body,
        ap_registrations_body,
    )
